...
$(function () {
	$("#hopperListTable").DataTable({
		...
		"drawCallback": function(settings) {
			/*$("#hopperListTable_next").remove();
			$("#hopperListTable_previous").remove();*/
			if (pagination) {
				$('#hopperListTable_paginate').find(".pagination").append('<li class="paginate_button page-item"><a onclick="loadNextSetValues()" id="loadMore_hopperListTable" class="fa fa-arrow-right page-link" href="#" title="Load More"></a></li>');
			}
		},
			...
			customize: function (doc) {
				...
				doc['header']=(function() { return {
					columns: [{
						alignment: 'left',
						fontSize: 15,
						text: "Six Sigma - EIM - ANAND Group",
					}, {
						alignment: 'right',
						image: logo,
						width: 100,
	...
	}).buttons().container().appendTo('#hopperListTable_wrapper .col-md-6:eq(0)');
	
	var loadAll = window.location.href.split("all").length;
	var hopperListTable = $("#hopperListTable").DataTable();
	if(hopperListTable.rows().count() == 0) {
		$(".dt-buttons").hide();
		$("#hopperListTable_filter").hide();
	}
	if (hopperListTable.rows().count() < 10 || loadAll > 1) {
		$("#loadMore_hopperListTable").hide();
		pagination = false;
	}
});
function loadNextSetValues() {
	var hopperListTable = $("#hopperListTable").DataTable();
	var offsetValue = hopperListTable.rows().count();
	$("#hopperListLoader").show();
	$('#hopperListTable_paginate').hide();
	$.ajax({
		url: 'loadMoreCompany',
		type: 'GET',
		...
		success: function(result) {
			if (result.length <= 20) {
				for (var i = 0; i < result.length; i++) {
					hopperListTable.row.add([
						result[i]["companyName"], result[i]["companyShortName"], result[i]["companyAcronym"],
						result[i]["companyActive"], result[i]["action"]
					]);
				}
				hopperListTable.draw(false);
				var currentPage = hopperListTable.page.info().page;
				var totalPages = hopperListTable.page.info().pages;
				console.log(totalPages);
				if (totalPages > 1) {
					if (currentPage != (totalPages - 1)) {
						hopperListTable.page(currentPage + 1).draw(false);
					}
				}
				$("#hopperListTable_paginate").scroll();
				$('#hopperListTable_paginate').show();
				if (result.length < 10) {
