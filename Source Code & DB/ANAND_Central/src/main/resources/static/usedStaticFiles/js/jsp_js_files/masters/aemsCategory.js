var pagination = true;
$(function() {
	$("#aemsCategoryTable").DataTable({
		"ordering": false,
		"paging": true,
		"info": true,
		"lengthChange": true,
		"searching": true,
		"autoWidth": false,
		"responsive": true,
		"drawCallback": function(settings) {
			if (pagination) {
				$('#aemsCategoryTable_paginate').find(".pagination").append('<li class="paginate_button page-item"><a onclick="loadNextSetValues()" id="loadMore_aemsCategoryTable" class="fa fa-arrow-right page-link" href="#" title="Load More"></a></li>');
			}
		},
		"createdRow": function (row,data,index) {
	    	if(index % 2 == 0) {
	    		$(row).addClass("bg-table-row-0");
	    	} else {
	    		$(row).addClass("bg-table-row-1");
	    	}
	    },
		"buttons": ["csv", "excel", {
			extend: 'pdf',
			className:'buttons-pdf',
			text:'PDF',
			titleAttr: 'Download in PDF',
			title:"Location List",
			orientation: 'portrait',
			customize: function (doc) {
				var now = new Date();
				var jsDate = now.getDate()+'-'+(now.getMonth()+1)+'-'+now.getFullYear();
				var logo = 'data:image/png;base64,';
				doc['header']=(function() { return {
					columns: [{
						alignment: 'left',
						fontSize: 15,
						text: "ANAND Projects",
					}, {
						alignment: 'right',
						image: logo,
						width: 100,
						height:20
					}],
					margin: 10
				}});
				doc['footer']=(function(page, pages) { return {
					columns: [{
						alignment: 'left',
						text: ['Created on: ', { text: jsDate.toString() }]
					}, {
						alignment: 'right',
						text: ['page ', { text: page.toString() },	' of ',	{ text: pages.toString() }]
					}],
					margin: 20
				}});
			},
			pageSize: 'LEGAL',
			exportOptions: {
				columns: ':visible thead th:not(.noExport)'
			}
		}, "print", "colvis"]
	}).buttons().container().appendTo('#aemsCategoryTable_wrapper .col-md-6:eq(0)');

	var aemsCategoryTable = $("#aemsCategoryTable").DataTable();
	var loadAll = window.location.href.split("all").length;
	if(aemsCategoryTable.rows().count() < 20 || loadAll > 1) {
		$("#loadMore_aemsCategoryTable").hide();
		pagination = false;
	}

	if($("#openAddModel").val() != "") {
		$(".btn-tool").click();
	}
	if($("#openEditModel").val() != "") {
		$("#closeEditaemsCategoryModal").attr("onclick", "location.href='aemsCategory'");
		$("#editaemsCategoryModal").modal({
		    backdrop: 'static',
		    keyboard: false
		});
	}
});
function loadNextSetValues() {
	var aemsCategoryTable = $("#aemsCategoryTable").DataTable();
	var offsetValue = aemsCategoryTable.rows().count();
	$('body').addClass('disableClickBody');
	$(".anand-loader").show();
	$.ajax({
		url:'loadMoreAemsCategory',
		type:'GET',
		data:{'offsetValue':offsetValue},
		dataType : 'json',
		success: function(result) {
			for(var i=0; i<result.length; i++) {
				aemsCategoryTable.row.add([
					result[i]["aemsCategoryAction"],result[i]["aemsCategoryName"],result[i]["aemsCategoryActive"],
					result[i]["aemsCategoryCreatedBy"],result[i]["aemsCategoryCreatedOn"],
					result[i]["aemsCategoryUpdatedBy"],result[i]["aemsCategoryUpdatedOn"]
				]);
			}
			aemsCategoryTable.draw(false);
			var currentPage = aemsCategoryTable.page.info().page;
			var totalPages = aemsCategoryTable.page.info().pages;
			console.log(totalPages);
			if(totalPages > 1) {
				if(currentPage != (totalPages-1)) {
					aemsCategoryTable.page(currentPage+1).draw(false);
				}
			}
			$("#aemsCategoryTable_paginate").scroll();
			$('#aemsCategoryTable_paginate').show();
			if(result.length < 20) {
				pagination = false;
				$('#loadMore_aemsCategoryTable').hide();
			}
			$('body').removeClass('disableClickBody');
			$(".anand-loader").hide();
		},
		error:function() {
			$('#aemsCategoryTable_paginate').show();
			$('body').removeClass('disableClickBody');
			$(".anand-loader").hide();
			alert("Something went wrong, kindly try again after sometime.");
		}
	});
}
function editAemsCategory(mAemsCategoryId) {
	$("body").addClass("disableClickBody");
	$(".anand-loader").show();
	$.ajax({
		url: "getAemsCategoryDetails",
		type: "GET",
		data: { "mAemsCategoryId": mAemsCategoryId },
		dataType: "json",
		success: function(result) {
			clearEditFormValidation();
			$("#mAemsCategoryId").val(mAemsCategoryId);
			$("#aemsCategoryNameEdit").val(result["aemsCategoryName"]);
			if(result["aemsCategoryActive"] == "1") {
				$("#aemsCategoryActiveEdit1").prop("checked", true);
			} else {
				$("#aemsCategoryActiveEdit2").prop("checked", true);
			}
			$("body").removeClass("disableClickBody");
			$(".anand-loader").hide();
			$("#editaemsCategoryModal").modal({
			    backdrop: 'static',
			    keyboard: false
			});
		},
		error: function() {
			alert("Something went wrong, try again later");
			$("body").removeClass("disableClickBody");
			$(".anand-loader").hide();
		}
	});
}
function clearEditFormValidation() {
	$("#aemsCategoryNameErrorEdit").html("");
	$("#aemsCategoryNameEdit").removeClass("is-valid");
	$("#aemsCategoryNameEdit").removeClass("is-invalid");

	$("#isActiveEditError").html("");
}
$("#aemsCategory").submit(function(){
	$("#submitAemsCategoryButton").addClass("disabledButton");
	$('body').addClass('disableClickBody');
	$(".anand-loader").show();
	var submitOrNot = true;
	if(aemsCategoryNameValidation("") == false) {
		submitOrNot = false;
	}
	if($("#aemsCategoryActive1").prop("checked") == false && $("#aemsCategoryActive2").prop("checked") == false) {
		$("#isActiveError").html("AEMS Category state must be selected");
		submitOrNot = false;
	}
	if(submitOrNot) {
		return true;
	} else {
		$("#submitAemsCategoryButton").removeClass("disabledButton");
		$('body').removeClass('disableClickBody');
		$(".anand-loader").hide();
		return false;
	}
});
$("#aemsCategoryEdit").submit(function(){
	$("#saveaemsCategoryChanges").addClass("disabledButton");
	$('body').addClass('disableClickBody');
	$(".anand-loader").show();
	var submitOrNot = true;
	if(aemsCategoryNameValidation("Edit") == false) {
		submitOrNot = false;
	}
	if($("#locationActiveEdit1").prop("checked") == false && $("#locationActiveEdit2").prop("checked") == false) {
		$("#isActiveEditError").html("AEMS Category state must be selected");
		submitOrNot = false;
	}
	if(submitOrNot) {
		return true;
	} else {
		$("#saveaemsCategoryChanges").removeClass("disabledButton");
		$('body').removeClass('disableClickBody');
		$(".anand-loader").hide();
		return false;
	}
});
$("#aemsCategoryName").focusout(function() {
	aemsCategoryNameValidation("");
});
$("#aemsCategoryNameEdit").focusout(function() {
	aemsCategoryNameValidation("Edit");
});
var aemsCategoryNameCheck = /^[ A-Za-z0-9-Á& ]*$/;
function aemsCategoryNameValidation(idAppend) {
	var id = "aemsCategoryName"+idAppend;
	var error = "aemsCategoryNameError"+idAppend;
	$("#"+id).removeClass("is-valid");
	$("#"+id).removeClass("is-invalid");
	$("#"+error).html("");
	var aemsCategoryName = $("#"+id).val().replace(/\s+/g, " ");
	if(aemsCategoryName != "") {
		if(aemsCategoryNameCheck.test(aemsCategoryName) == false) {
			$("#"+error).html("AEMS Category Name cannot contain special characters.");
			$("#"+id).addClass("is-invalid");
			return false;
		} else {
			$("#"+id).addClass("is-valid");
			return true;
		}
	} else {
		$("#"+error).html("AEMS Category Name cannot be empty.");
		$("#"+id).addClass("is-invalid");
		return false;
	}
}
$("#aemsCategoryDescription").focusout(function() {
	aemsCategoryDescriptionValidation("");
});
$("#aemsCategoryDescriptionEdit").focusout(function() {
	aemsCategoryDescriptionValidation("Edit");
});
var aemsCategoryDescriptionCheck = /^[ A-Za-z0-9-Á& ]*$/;
function aemsCategoryDescriptionValidation(idAppend) {
	var id = "aemsCategoryDescription"+idAppend;
	var error = "aemsCategoryDescriptionError"+idAppend;
	$("#"+id).removeClass("is-valid");
	$("#"+id).removeClass("is-invalid");
	$("#"+error).html("");
	var aemsCategoryDescription = $("#"+id).val().replace(/\s+/g, " ");
	if(aemsCategoryDescription != "") {
		if(aemsCategoryDescriptionCheck.test(aemsCategoryDescription) == false) {
			$("#"+error).html("aemsCategory Description cannot contain special characters.");
			$("#"+id).addClass("is-invalid");
			return false;
		} else {
			$("#"+id).addClass("is-valid");
			return true;
		}
	} else {
		$("#"+error).html("AEMS Category Description cannot be empty.");
		$("#"+id).addClass("is-invalid");
		return false;
	}
}
$('#isActiveDiv input[type="radio"]').click(function() {
	$("#isActiveError").html("");
});
$('#isActiveEditDiv input[type="radio"]').click(function() {
	$("#isActiveEditError").html("");
});