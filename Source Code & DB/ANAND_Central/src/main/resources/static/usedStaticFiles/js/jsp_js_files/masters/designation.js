var pagination = true;
$(function() {
	$("#designationTable").DataTable({
		"ordering": false,
		"paging": true,
		"info": true,
		"lengthChange": false,
		"searching": true,
		"autoWidth": false,
		"responsive": true,
		"drawCallback": function(settings) {
			if (pagination) {
				$('#designationTable_paginate').find(".pagination").append('<li class="paginate_button page-item"><a onclick="loadNextSetValues()" id="loadMore_designationTable" class="fa fa-arrow-right page-link" href="#" title="Load More"></a></li>');
			}
		},
		"createdRow": function (row,data,index) {
	    	if(index % 2 == 0) {
	    		$(row).addClass("bg-table-row-0");
	    	} else {
	    		$(row).addClass("bg-table-row-1");
	    	}
	    },
		"buttons": ["csv", "excel", {
			extend: 'pdf',
			className:'buttons-pdf',
			text:'PDF',
			titleAttr: 'Download in PDF',
			title:"Location List",
			orientation: 'portrait',
			customize: function (doc) {
				var now = new Date();
				var jsDate = now.getDate()+'-'+(now.getMonth()+1)+'-'+now.getFullYear();
				var logo = 'data:image/png;base64,';
				doc['header']=(function() { return {
					columns: [{
						alignment: 'left',
						fontSize: 15,
						text: "ANAND Projects",
					}, {
						alignment: 'right',
						image: logo,
						width: 100,
						height:20
					}],
					margin: 10
				}});
				doc['footer']=(function(page, pages) { return {
					columns: [{
						alignment: 'left',
						text: ['Created on: ', { text: jsDate.toString() }]
					}, {
						alignment: 'right',
						text: ['page ', { text: page.toString() },	' of ',	{ text: pages.toString() }]
					}],
					margin: 20
				}});
			},
			pageSize: 'LEGAL',
			exportOptions: {
				columns: ':visible thead th:not(.noExport)'
			}
		}, "print", "colvis"]
	}).buttons().container().appendTo('#designationTable_wrapper .col-md-6:eq(0)');

	var designationTable = $("#designationTable").DataTable();
	var loadAll = window.location.href.split("all").length;
	if(designationTable.rows().count() < 20 || loadAll > 1) {
		$("#loadMore_designationTable").hide();
		pagination = false;
	}

	if($("#openAddModel").val() != "") {
		$(".btn-tool").click();
	}
	if($("#openEditModel").val() != "") {
		$("#closeEditDesignationModal").attr("onclick", "location.href='designation'");
		$("#editDesignationModal").modal({
		    backdrop: 'static',
		    keyboard: false
		});
	}
});
function loadNextSetValues() {
	var designationTable = $("#designationTable").DataTable();
	var offsetValue = designationTable.rows().count();
	$('body').addClass('disableClickBody');
	$(".anand-loader").show();
	$.ajax({
		url:'loadMoreDesignation',
		type:'GET',
		data:{'offsetValue':offsetValue},
		dataType : 'json',
		success: function(result) {
			for(var i=0; i<result.length; i++) {
				designationTable.row.add([
					result[i]["designationAction"],result[i]["designationName"],result[i]["designationActive"],
					result[i]["designationCreatedBy"],result[i]["designationCreatedOn"],
					result[i]["designationUpdatedBy"],result[i]["designationUpdatedOn"]
				]);
			}
			designationTable.draw(false);
			var currentPage = designationTable.page.info().page;
			var totalPages = designationTable.page.info().pages;
			console.log(totalPages);
			if(totalPages > 1) {
				if(currentPage != (totalPages-1)) {
					designationTable.page(currentPage+1).draw(false);
				}
			}
			$("#designationTable_paginate").scroll();
			$('#designationTable_paginate').show();
			if(result.length < 20) {
				/*pagination = false;
				$('#loadMore_designationTable').hide();*/
			}
			$('body').removeClass('disableClickBody');
			$(".anand-loader").hide();
		},
		error:function() {
			$('#designationTable_paginate').show();
			$('body').removeClass('disableClickBody');
			$(".anand-loader").hide();
			alert("Something went wrong, kindly try again after sometime.");
		}
	});
}
function editDesignation(mDesignationId) {
	$("body").addClass("disableClickBody");
	$(".anand-loader").show();
	$.ajax({
		url: "getDesignationDetails",
		type: "GET",
		data: { "mDesignationId": mDesignationId },
		dataType: "json",
		success: function(result) {
			clearEditFormValidation();
			$("#mDesignationId").val(mDesignationId);
			$("#designationNameEdit").val(result["designationName"]);
			$("#designationDescriptionEdit").val(result["designationDescription"]);
			if(result["designationActive"] == "1") {
				$("#designationActiveEdit1").prop("checked", true);
			} else {
				$("#designationActiveEdit2").prop("checked", true);
			}
			$("body").removeClass("disableClickBody");
			$(".anand-loader").hide();
			$("#editDesignationModal").modal({
			    backdrop: 'static',
			    keyboard: false
			});
		},
		error: function() {
			alert("Something went wrong, try again later");
			$("body").removeClass("disableClickBody");
			$(".anand-loader").hide();
		}
	});
}
function clearEditFormValidation() {
	$("#designationNameErrorEdit").html("");
	$("#designationNameEdit").removeClass("is-valid");
	$("#designationNameEdit").removeClass("is-invalid");

	$("#designationDescriptionErrorEdit").html("");
	$("#designationDescriptionEdit").removeClass("is-valid");
	$("#designationDescriptionEdit").removeClass("is-invalid");

	$("#isActiveEditError").html("");
}
$("#designation").submit(function(){
	$("#submitDesignationButton").addClass("disabledButton");
	$('body').addClass('disableClickBody');
	$(".anand-loader").show();
	var submitOrNot = true;
	if(designationNameValidation("") == false) {
		submitOrNot = false;
	}
	if($("#designationActive1").prop("checked") == false && $("#designationActive2").prop("checked") == false) {
		$("#isActiveError").html("Designation state must be selected");
		submitOrNot = false;
	}
	if(submitOrNot) {
		return true;
	} else {
		$("#submitDesignationButton").removeClass("disabledButton");
		$('body').removeClass('disableClickBody');
		$(".anand-loader").hide();
		return false;
	}
});
$("#designationEdit").submit(function(){
	$("#saveDesignationChanges").addClass("disabledButton");
	$('body').addClass('disableClickBody');
	$(".anand-loader").show();
	var submitOrNot = true;
	if(designationNameValidation("Edit") == false) {
		submitOrNot = false;
	}
	if($("#locationActiveEdit1").prop("checked") == false && $("#locationActiveEdit2").prop("checked") == false) {
		$("#isActiveEditError").html("Designation state must be selected");
		submitOrNot = false;
	}
	if(submitOrNot) {
		return true;
	} else {
		$("#saveDesignationChanges").removeClass("disabledButton");
		$('body').removeClass('disableClickBody');
		$(".anand-loader").hide();
		return false;
	}
});
$("#designationName").focusout(function() {
	designationNameValidation("");
});
$("#designationNameEdit").focusout(function() {
	designationNameValidation("Edit");
});
var designationNameCheck = /^[ A-Za-z0-9-Á& ]*$/;
function designationNameValidation(idAppend) {
	var id = "designationName"+idAppend;
	var error = "designationNameError"+idAppend;
	$("#"+id).removeClass("is-valid");
	$("#"+id).removeClass("is-invalid");
	$("#"+error).html("");
	var designationName = $("#"+id).val().replace(/\s+/g, " ");
	if(designationName != "") {
		if(designationNameCheck.test(designationName) == false) {
			$("#"+error).html("Designation Name cannot contain special characters.");
			$("#"+id).addClass("is-invalid");
			return false;
		} else {
			$("#"+id).addClass("is-valid");
			return true;
		}
	} else {
		$("#"+error).html("Designation Name cannot be empty.");
		$("#"+id).addClass("is-invalid");
		return false;
	}
}
$("#designationDescription").focusout(function() {
	designationDescriptionValidation("");
});
$("#designationDescriptionEdit").focusout(function() {
	designationDescriptionValidation("Edit");
});
var designationDescriptionCheck = /^[ A-Za-z0-9-Á& ]*$/;
function designationDescriptionValidation(idAppend) {
	var id = "designationDescription"+idAppend;
	var error = "designationDescriptionError"+idAppend;
	$("#"+id).removeClass("is-valid");
	$("#"+id).removeClass("is-invalid");
	$("#"+error).html("");
	var designationDescription = $("#"+id).val().replace(/\s+/g, " ");
	if(designationDescription != "") {
		if(designationDescriptionCheck.test(designationDescription) == false) {
			$("#"+error).html("Designation Description cannot contain special characters.");
			$("#"+id).addClass("is-invalid");
			return false;
		} else {
			$("#"+id).addClass("is-valid");
			return true;
		}
	} else {
		$("#"+error).html("Designation Description cannot be empty.");
		$("#"+id).addClass("is-invalid");
		return false;
	}
}
$('#isActiveDiv input[type="radio"]').click(function() {
	$("#isActiveError").html("");
});
$('#isActiveEditDiv input[type="radio"]').click(function() {
	$("#isActiveEditError").html("");
});