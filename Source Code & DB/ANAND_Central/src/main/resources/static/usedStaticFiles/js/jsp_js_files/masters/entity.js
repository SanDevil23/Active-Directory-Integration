...
$(function() {
	...
	if(entityTable.rows().count() < 20 || loadAll > 1) {
		...
		success: function(result) {
			for(var i=0; i<result.length; i++) {
				entityTable.row.add([
					...
					result[i]["entityShortName"],result[i]["entitySource"],
					result[i]["entityLocation"],result[i]["entityZone"],
...
var entityNameCheck = /^[ A-Za-z0-9-Á& ]*$/;
function entityNameValidation(idAppend) {
	...
	if(entityName != "") {
		if(entityNameCheck.test(entityName) == false) {
...
var entityShortNameCheck = /^[ A-Za-z0-9-Á& ]*$/;
function entityShortNameValidation(idAppend) {
	...
	if(entityShortName != "") {
		if(entityShortNameCheck.test(entityShortName) == false) {
...
var entityAcronymCheck = /^[ A-Za-z0-9-Á& ]*$/;
function entityAcronymValidation(idAppend) {
	...
	if(entityAcronym != "") {
		if(entityAcronymCheck.test(entityAcronym) == false) {
