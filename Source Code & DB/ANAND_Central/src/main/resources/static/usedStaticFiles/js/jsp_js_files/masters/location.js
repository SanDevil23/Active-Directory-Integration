var pagination = true;
$(function() {
	$("#locationTable").DataTable({
		"ordering": false,
		"paging": true,
		"info": true,
		"scrollX": true,
		"lengthChange": true,
		"searching": true,
		"autoWidth": false,
		"responsive": false,
		"drawCallback": function(settings) {
			if (pagination) {
				$('#locationTable_paginate').find(".pagination").append('<li class="paginate_button page-item"><a onclick="loadNextSetValues()" id="loadMore_locationTable" class="fa fa-arrow-right page-link" href="#" title="Load More"></a></li>');
			}
		},
		"createdRow": function (row,data,index) {
	    	if(index % 2 == 0) {
	    		$(row).addClass("bg-table-row-0");
	    	} else {
	    		$(row).addClass("bg-table-row-1");
	    	}
	    },
		"buttons": ["csv", "excel", {
			extend: 'pdf',
			className:'buttons-pdf',
			text:'PDF',
			titleAttr: 'Download in PDF',
			title:"Entity List",
			orientation: 'portrait',
			customize: function (doc) {
				var now = new Date();
				var jsDate = now.getDate()+'-'+(now.getMonth()+1)+'-'+now.getFullYear();
				var logo = 'data:image/png;base64,';
				doc['header']=(function() { return {
					columns: [{
						alignment: 'left',
						fontSize: 15,
						text: "ANAND Projects",
					}, {
						alignment: 'right',
						image: logo,
						width: 100,
						height:20
					}],
					margin: 10
				}});
				doc['footer']=(function(page, pages) { return {
					columns: [{
						alignment: 'left',
						text: ['Created on: ', { text: jsDate.toString() }]
					}, {
						alignment: 'right',
						text: ['page ', { text: page.toString() },	' of ',	{ text: pages.toString() }]
					}],
					margin: 20
				}});
			},
			pageSize: 'LEGAL',
			exportOptions: {
				columns: ':visible thead th:not(.noExport)'
			}
		}, "print", "colvis"]
	}).buttons().container().appendTo('#locationTable_wrapper .col-md-6:eq(0)');

	var locationTable = $("#locationTable").DataTable();
	var loadAll = window.location.href.split("all").length;
	if(locationTable.rows().count() < 20 || loadAll > 1) {
		$("#loadMore_locationTable").hide();
		pagination = false;
	}

	if($("#openAddModel").val() != "") {
		$(".btn-tool").click();
	}
	if($("#openEditModel").val() != "") {
		$("#closeEditLocationModal").attr("onclick", "location.href='location'");
		$("#editLocationModal").modal({
		    backdrop: 'static',
		    keyboard: false
		});
	}
});
function loadNextSetValues() {
	var locationTable = $("#locationTable").DataTable();
	var offsetValue = locationTable.rows().count();
	$('body').addClass('disableClickBody');
	$(".anand-loader").show();
	$.ajax({
		url:'loadMoreLocation',
		type:'GET',
		data:{'offsetValue':offsetValue},
		dataType : 'json',
		success: function(result) {
			for(var i=0; i<result.length; i++) {
				locationTable.row.add([
					result[i]["locationAction"],result[i]["locationName"],result[i]["locationZone"],
					result[i]["locationActive"],result[i]["locationCreatedBy"],result[i]["locationCreatedOn"],
					result[i]["locationUpdatedBy"],result[i]["locationUpdatedOn"]
				]);
			}
			locationTable.draw(false);
			var currentPage = locationTable.page.info().page;
			var totalPages = locationTable.page.info().pages;
			console.log(totalPages);
			if(totalPages > 1) {
				if(currentPage != (totalPages-1)) {
					locationTable.page(currentPage+1).draw(false);
				}
			}
			$("#locationTable_paginate").scroll();
			$('#locationTable_paginate').show();
			if(result.length < 20) {
				pagination = false;
				$('#loadMore_locationTable').hide();
			}
			$('body').removeClass('disableClickBody');
			$(".anand-loader").hide();
		},
		error:function() {
			$('#locationTable_paginate').show();
			$('body').removeClass('disableClickBody');
			$(".anand-loader").hide();
			alert("Something went wrong, kindly try again after sometime.");
		}
	});
}
function editLocation(mLocationId) {
	$("body").addClass("disableClickBody");
	$(".anand-loader").show();
	$.ajax({
		url: "getLocationDetails",
		type: "GET",
		data: { "mLocationId": mLocationId },
		dataType: "json",
		success: function(result) {
			clearEditFormValidation();
			$("#mLocationId").val(mLocationId);
			$("#locationNameEdit").val(result["locationName"]);
			if(result["locationActive"] == "1") {
				$("#locationActiveEdit1").prop("checked", true);
			} else {
				$("#locationActiveEdit2").prop("checked", true);
			}
			$("#zoneIdTempEdit").val(result["zoneIdTempEdit"])
			$("body").removeClass("disableClickBody");
			$(".anand-loader").hide();
			$("#editLocationModal").modal({
			    backdrop: 'static',
			    keyboard: false
			});
		},
		error: function() {
			alert("Something went wrong, try again later");
		}
	});
}
function clearEditFormValidation() {
	$("#locationNameErrorEdit").html("");
	$("#locationNameEdit").removeClass("is-valid");
	$("#locationNameEdit").removeClass("is-invalid");

	$("#zoneIdTempErrorEdit").html("");
	$("#zoneIdTempEdit").removeClass("is-valid");
	$("#zoneIdTempEdit").removeClass("is-invalid");
	
	$("#isActiveEditError").html("");
}
$("#location").submit(function(){
	$("#submitLocationButton").addClass("disabledButton");
	$('body').addClass('disableClickBody');
	$(".anand-loader").show();
	var submitOrNot = true;
	if(locationNameValidation("") == false) {
		submitOrNot = false;
	}
	if(zoneIdTempValidation("") == false) {
		submitOrNot = false;
	}
	if($("#locationActive1").prop("checked") == false && $("#locationActive2").prop("checked") == false) {
		$("#isActiveError").html("Location state must be selected");
		submitOrNot = false;
	}
	if(submitOrNot) {
		return true;
	} else {
		$("#submitLocationButton").removeClass("disabledButton");
		$('body').removeClass('disableClickBody');
		$(".anand-loader").hide();
		return false;
	}
});
$("#locationEdit").submit(function(){
	$("#saveLocationChanges").addClass("disabledButton");
	$('body').addClass('disableClickBody');
	$(".anand-loader").show();
	var submitOrNot = true;
	if(locationNameValidation("Edit") == false) {
		submitOrNot = false;
	}
	if(zoneIdTempValidation("Edit") == false) {
		submitOrNot = false;
	}
	if($("#locationActiveEdit1").prop("checked") == false && $("#locationActiveEdit2").prop("checked") == false) {
		$("#isActiveEditError").html("Location state must be selected");
		submitOrNot = false;
	}
	if(submitOrNot) {
		return true;
	} else {
		$("#saveLocationChanges").removeClass("disabledButton");
		$('body').removeClass('disableClickBody');
		$(".anand-loader").hide();
		return false;
	}
});
$("#locationName").focusout(function() {
	locationNameValidation("");
});
$("#locationNameEdit").focusout(function() {
	locationNameValidation("Edit");
});
var locationNameCheck = /^[ A-Za-z0-9-Á& ]*$/;
function locationNameValidation(idAppend) {
	var id = "locationName"+idAppend;
	var error = "locationNameError"+idAppend;
	$("#"+id).removeClass("is-valid");
	$("#"+id).removeClass("is-invalid");
	$("#"+error).html("");
	var locationName = $("#"+id).val().replace(/\s+/g, " ");
	if(locationName != "") {
		if(locationNameCheck.test(locationName) == false) {
			$("#"+error).html("Location Name cannot contain special characters.");
			$("#"+id).addClass("is-invalid");
			return false;
		} else {
			$("#"+id).addClass("is-valid");
			return true;
		}
	} else {
		$("#"+error).html("Location Name cannot be empty.");
		$("#"+id).addClass("is-invalid");
		return false;
	}
}
$("#zoneIdTemp").change(function() {
	zoneIdTempValidation("");
});
$("#zoneIdTempEdit").change(function() {
	zoneIdTempValidation("Edit");
});
function zoneIdTempValidation(idAppend) {
	var id = "zoneIdTemp"+idAppend;
	var error = "zoneIdTempError"+idAppend;
	$("#"+id).removeClass("is-valid");
	$("#"+id).removeClass("is-invalid");
	$("#"+error).html("");
	var zoneIdTemp = $("#"+id).val();
	if(zoneIdTemp != "-1") {
		$("#"+id).addClass("is-valid");
		return true;
	} else {
		$("#"+error).html("Zone cannot be empty.");
		$("#"+id).addClass("is-invalid");
		return false;
	}
}
$('#isActiveDiv input[type="radio"]').click(function() {
	$("#isActiveError").html("");
});
$('#isActiveEditDiv input[type="radio"]').click(function() {
	$("#isActiveEditError").html("");
});