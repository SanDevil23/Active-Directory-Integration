var pagination = true;
$(function() {
	$("#roleTable").DataTable({
		"ordering": false,
		"paging": true,
		"info": true,
		"lengthChange": true,
		"searching": true,
		"autoWidth": false,
		"responsive": true,
		"drawCallback": function(settings) {
			if (pagination) {
				$('#roleTable_paginate').find(".pagination").append('<li class="paginate_button page-item"><a onclick="loadNextSetValues()" id="loadMore_roleTable" class="fa fa-arrow-right page-link" href="#" title="Load More"></a></li>');
			}
		},
		"createdRow": function (row,data,index) {
	    	if(index % 2 == 0) {
	    		$(row).addClass("bg-table-row-0");
	    	} else {
	    		$(row).addClass("bg-table-row-1");
	    	}
	    },
		"buttons": ["csv", "excel", {
			extend: 'pdf',
			className:'buttons-pdf',
			text:'PDF',
			titleAttr: 'Download in PDF',
			title:"Role List",
			orientation: 'portrait',
			customize: function (doc) {
				var now = new Date();
				var jsDate = now.getDate()+'-'+(now.getMonth()+1)+'-'+now.getFullYear();
				var logo = 'data:image/png;base64,';
				doc['header']=(function() { return {
					columns: [{
						alignment: 'left',
						fontSize: 15,
						text: "ANAND Projects",
					}, {
						alignment: 'right',
						image: logo,
						width: 100,
						height:20
					}],
					margin: 10
				}});
				doc['footer']=(function(page, pages) { return {
					columns: [{
						alignment: 'left',
						text: ['Created on: ', { text: jsDate.toString() }]
					}, {
						alignment: 'right',
						text: ['page ', { text: page.toString() },	' of ',	{ text: pages.toString() }]
					}],
					margin: 20
				}});
			},
			pageSize: 'LEGAL',
			exportOptions: {
				columns: ':visible thead th:not(.noExport)'
			}
		}, "print", "colvis"]
	}).buttons().container().appendTo('#roleTable_wrapper .col-md-6:eq(0)');

	var roleTable = $("#roleTable").DataTable();
	var loadAll = window.location.href.split("all").length;
	if(roleTable.rows().count() < 20 || loadAll > 1) {
		$("#loadMore_roleTable").hide();
		pagination = false;
	}

	if($("#openAddModel").val() != "") {
		$(".btn-tool").click();
	}
	if($("#openEditModel").val() != "") {
		$("#closeEditRoleModal").attr("onclick", "location.href='role'");
		$("#editRoleModal").modal({
		    backdrop: 'static',
		    keyboard: false
		});
	}
});
function loadNextSetValues() {
	var roleTable = $("#roleTable").DataTable();
	var offsetValue = roleTable.rows().count();
	$('body').addClass('disableClickBody');
	$(".anand-loader").show();
	$.ajax({
		url:'loadMoreRole',
		type:'GET',
		data:{'offsetValue':offsetValue},
		dataType : 'json',
		success: function(result) {
			for(var i=0; i<result.length; i++) {
				roleTable.row.add([
					result[i]["roleAction"],result[i]["roleName"],
					result[i]["roleDescription"],result[i]["roleActive"],
					result[i]["roleCreatedBy"],result[i]["roleCreatedOn"],
					result[i]["roleUpdatedBy"],result[i]["roleUpdatedOn"]
				]);
			}
			roleTable.draw(false);
			var currentPage = roleTable.page.info().page;
			var totalPages = roleTable.page.info().pages;
			console.log(totalPages);
			if(totalPages > 1) {
				if(currentPage != (totalPages-1)) {
					roleTable.page(currentPage+1).draw(false);
				}
			}
			$("#roleTable_paginate").scroll();
			$('#roleTable_paginate').show();
			if(result.length < 20) {
				pagination = false;
				$('#loadMore_roleTable').hide();
			}
			$('body').removeClass('disableClickBody');
			$(".anand-loader").hide();
		},
		error:function() {
			$('#roleTable_paginate').show();
			$('body').removeClass('disableClickBody');
			$(".anand-loader").hide();
			alert("Something went wrong, kindly try again after sometime.");
		}
	});
}
function editRole(mRoleId) {
	$('body').addClass('disableClickBody');
	$(".anand-loader").show();
	$.ajax({
		url: 'getRoleDetails',
		type: 'GET',
		data: { 'mRoleId': mRoleId },
		dataType: 'json',
		success: function(result) {
			clearEditFormValidation();
			$("#mRoleId").val(mRoleId);
			$("#roleNameEdit").val(result["roleName"]);
			$("#roleDescriptionEdit").val(result["roleDescription"]);
			if(result["roleActive"] == "1") {
				$("#roleActiveEdit1").prop('checked', true);
			} else {
				$("#roleActiveEdit2").prop('checked', true);
			}
			$('body').removeClass('disableClickBody');
			$(".anand-loader").hide();
			$("#editRoleModal").modal({
			    backdrop: 'static',
			    keyboard: false
			});
		},
		error: function() {
			alert("Something went wrong, try again later");
		}
	});
}
function clearEditFormValidation() {
	$("#roleNameEdit").html("");
	$("#roleNameEdit").removeClass("is-valid");
	$("#roleNameEdit").removeClass("is-invalid");
	
	$("#isActiveEditError").html("");
	$("#isActiveEditError").removeClass("is-invalid");
	$("#isActiveEditError").removeClass("is-valid");
}
$("#role").submit(function(){
	$("#submitRoleButton").addClass("disabledButton");
	$('body').addClass('disableClickBody');
	$(".anand-loader").show();
	var submitOrNot = true;
	if(roleNameValidation("") == false) {
		submitOrNot = false;
	}
	if(roleDescriptionValidation("") == false) {
		submitOrNot = false;
	}
	if($("#roleActive1").prop("checked") == false && $("#roleActive2").prop("checked") == false) {
		$("#isActiveError").html("Role state must be selected");
		submitOrNot = false;
	}
	if(submitOrNot) {
		return true;
	} else {
		$("#submitRoleButton").removeClass("disabledButton");
		$('body').removeClass('disableClickBody');
		$(".anand-loader").hide();
		return false;
	}
});
$("#roleEdit").submit(function(){
	$("#saveRoleChanges").addClass("disabledButton");
	$('body').addClass('disableClickBody');
	$(".anand-loader").show();
	var submitOrNot = true;
	if(roleNameValidation("Edit") == false) {
		submitOrNot = false;
	}
	if(roleDescriptionValidation("Edit") == false) {
		submitOrNot = false;
	}
	if($("#roleActiveEdit1").prop("checked") == false && $("#roleActiveEdit2").prop("checked") == false) {
		$("#isActiveError").html("Role state must be selected");
		submitOrNot = false;
	}
	if(submitOrNot) {
		return true;
	} else {
		$("#saveRoleChanges").removeClass("disabledButton");
		$('body').removeClass('disableClickBody');
		$(".anand-loader").hide();
		return false;
	}
});
$("#roleName").focusout(function() {
	roleNameValidation("");
});
$("#roleNameEdit").focusout(function() {
	roleNameValidation("Edit");
});
var roleNameCheck = /^[ A-Za-z0-9-Á& ]*$/;
function roleNameValidation(idAppend) {
	var id = "roleName"+idAppend;
	var error = "roleNameError"+idAppend;
	$("#"+id).removeClass("is-valid");
	$("#"+id).removeClass("is-invalid");
	$("#"+error).html("");
	var roleName = $("#"+id).val().replace(/\s+/g, " ");
	if(roleName != "") {
		if(roleNameCheck.test(roleName) == false) {
			$("#"+error).html("Role Name cannot contain special characters.");
			$("#"+id).addClass("is-invalid");
			return false;
		} else {
			$("#"+id).addClass("is-valid");
			return true;
		}
	} else {
		$("#"+error).html("Role Name cannot be empty.");
		$("#"+id).addClass("is-invalid");
		return false;
	}
}
$("#roleDescription").focusout(function() {
	roleDescriptionValidation("");
});
$("#roleDescriptionEdit").focusout(function() {
	roleDescriptionValidation("Edit");
});
var roleDescriptionCheck = /^[ A-Za-z0-9-Á& ]*$/;
function roleDescriptionValidation(idAppend) {
	var id = "roleDescription"+idAppend;
	var error = "roleDescriptionError"+idAppend;
	$("#"+id).removeClass("is-valid");
	$("#"+id).removeClass("is-invalid");
	$("#"+error).html("");
	var roleDescription = $("#"+id).val().replace(/\s+/g, " ");
	if(roleDescription != "") {
		if(roleDescriptionCheck.test(roleDescription) == false) {
			$("#"+error).html("Role Description cannot contain special characters.");
			$("#"+id).addClass("is-invalid");
			return false;
		} else {
			$("#"+id).addClass("is-valid");
			return true;
		}
	} else {
		$("#"+error).html("Role Description cannot be empty.");
		$("#"+id).addClass("is-invalid");
		return false;
	}
}
$('#isActiveDiv input[type="radio"]').click(function() {
	$("#isActiveError").html("");
});
$('#isActiveEditDiv input[type="radio"]').click(function() {
	$("#isActiveEditError").html("");
});