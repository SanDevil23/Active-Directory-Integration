	...
	$.ajax({
		...
		success: function(result) {
			for(var i=0; i<result.length; i++) {
				userTable.row.add([
					result[i]["userAction"],result[i]["userFirstName"],
					result[i]["userMiddleName"],result[i]["userLastName"],
					result[i]["userEntity"],result[i]["userLocation"],
...
function editUserInfo(mUserId) {
	...
	$.ajax({
		...
		success: function(result) {
			...
			$("#tempEntityIdEdit").val(result["userEntityId"]);
			$("#tempLocationIdEdit").val(result["userEntityLocationId"]);
			$("#tempDesignationIdEdit").val(result["userDesignationId"]);
			$("#tempDepartmentIdEdit").val(result["userDepartmentId"]);
			...
			$("#userRolesTemp").val(userRolesTemp).trigger("change");
			
			alert(result["userApps"]);
			var userAppsTemp = result["userApps"].split(",");
			$("#userAppsTemp").val(userAppsTemp).trigger("change");
...
function clearEditFormValidation() {
	...
	$("#userReportingManagerMailEditError").html("");
