var pagination = true;
$(function() {
	$("#zoneMasterTable").DataTable({
		"ordering": false,
		"paging": true,
		"info": true,
		"scrollX": true,
		"lengthChange": true,
		"searching": true,
		"autoWidth": false,
		"responsive": false,
		"drawCallback": function(settings) {
			if (pagination) {
				$('#zoneMasterTable_paginate').find(".pagination").append('<li class="paginate_button page-item"><a onclick="loadNextSetValues()" id="loadMore_zoneMasterTable" class="fa fa-arrow-right page-link" href="#" title="Load More"></a></li>');
			}
		},
		"createdRow": function (row,data,index) {
	    	if(index % 2 == 0) {
	    		$(row).addClass("bg-table-row-0");
	    	} else {
	    		$(row).addClass("bg-table-row-1");
	    	}
	    },
		"buttons": ["csv", "excel", {
			extend: 'pdf',
			className:'buttons-pdf',
			text:'PDF',
			titleAttr: 'Download in PDF',
			title:"Entity List",
			orientation: 'portrait',
			customize: function (doc) {
				var now = new Date();
				var jsDate = now.getDate()+'-'+(now.getMonth()+1)+'-'+now.getFullYear();
				var logo = 'data:image/png;base64,';
				doc['header']=(function() { return {
					columns: [{
						alignment: 'left',
						fontSize: 15,
						text: "ANAND Projects",
					}, {
						alignment: 'right',
						image: logo,
						width: 100,
						height:20
					}],
					margin: 10
				}});
				doc['footer']=(function(page, pages) { return {
					columns: [{
						alignment: 'left',
						text: ['Created on: ', { text: jsDate.toString() }]
					}, {
						alignment: 'right',
						text: ['page ', { text: page.toString() },	' of ',	{ text: pages.toString() }]
					}],
					margin: 20
				}});
			},
			pageSize: 'LEGAL',
			exportOptions: {
				columns: ':visible thead th:not(.noExport)'
			}
		}, "print", "colvis"]
	}).buttons().container().appendTo('#zoneMasterTable_wrapper .col-md-6:eq(0)');

	var zoneMasterTable = $("#zoneMasterTable").DataTable();
	var loadAll = window.location.href.split("all").length;
	if(zoneMasterTable.rows().count() < 20 || loadAll > 1) {
		$("#loadMore_zoneMasterTable").hide();
		pagination = false;
	}

	if($("#openAddModel").val() != "") {
		$(".btn-tool").click();
	}
	if($("#openEditModel").val() != "") {
		$("#closeEditZoneModal").attr("onclick", "location.href='zone'");
		$("#editZoneModal").modal({
		    backdrop: 'static',
		    keyboard: false
		});
	}
});
function loadNextSetValues() {
	var zoneMasterTable = $("#zoneMasterTable").DataTable();
	var offsetValue = zoneMasterTable.rows().count();
	$('body').addClass('disableClickBody');
	$(".anand-loader").show();
	$.ajax({
		url:'loadMoreZone',
		type:'GET',
		data:{'offsetValue':offsetValue},
		dataType : 'json',
		success: function(result) {
			for(var i=0; i<result.length; i++) {
				zoneMasterTable.row.add([
					result[i]["zoneAction"],result[i]["zoneName"],result[i]["zoneActive"],
					result[i]["zoneCreatedBy"],result[i]["zoneCreatedOn"],
					result[i]["zoneUpdatedBy"],result[i]["zoneUpdatedOn"]
				]);
			}
			zoneMasterTable.draw(false);
			var currentPage = zoneMasterTable.page.info().page;
			var totalPages = zoneMasterTable.page.info().pages;
			console.log(totalPages);
			if(totalPages > 1) {
				if(currentPage != (totalPages-1)) {
					zoneMasterTable.page(currentPage+1).draw(false);
				}
			}
			$("#zoneMasterTable_paginate").scroll();
			$('#zoneMasterTable_paginate').show();
			if(result.length < 20) {
				pagination = false;
				$('#loadMore_zoneMasterTable').hide();
			}
			$('body').removeClass('disableClickBody');
			$(".anand-loader").hide();
		},
		error:function() {
			$('#zoneMasterTable_paginate').show();
			$('body').removeClass('disableClickBody');
			$(".anand-loader").hide();
			alert("Something went wrong, kindly try again after sometime.");
		}
	});
}
function editZone(mZoneId) {
	$('body').addClass('disableClickBody');
	$(".anand-loader").show();
	$.ajax({
		url: 'getZoneDetails',
		type: 'GET',
		data: { 'mZoneId': mZoneId },
		dataType: 'json',
		success: function(result) {
			clearEditFormValidation();
			$("#mZoneId").val(mZoneId);
			$("#zoneNameEdit").val(result["zoneName"]);
			if(result["zoneActive"] == "1") {
				$("#zoneActiveEdit1").prop('checked', true);
			} else {
				$("#zoneActiveEdit2").prop('checked', true);
			}
			$('body').removeClass('disableClickBody');
			$(".anand-loader").hide();
			$("#editZoneModal").modal({
			    backdrop: 'static',
			    keyboard: false
			});
		},
		error: function() {
			alert("Something went wrong, try again later");
		}
	});
}
function clearEditFormValidation() {
	$("#zoneNameEdit").html("");
	$("#zoneNameEdit").removeClass("is-valid");
	$("#zoneNameEdit").removeClass("is-invalid");
	
	$("#isActiveEditError").html("");
	$("#isActiveEditError").removeClass("is-invalid");
	$("#isActiveEditError").removeClass("is-valid");
}
$("#zone").submit(function(){
	$("#submitZoneButton").addClass("disabledButton");
	$('body').addClass('disableClickBody');
	$(".anand-loader").show();
	var submitOrNot = true;
	if(zoneNameValidation("") == false) {
		submitOrNot = false;
	}
	if($("#zoneActive1").prop("checked") == false && $("#zoneActive2").prop("checked") == false) {
		$("#isActiveError").html("Zone state must be selected");
		submitOrNot = false;
	}
	if(submitOrNot) {
		return true;
	} else {
		$("#submitZoneButton").removeClass("disabledButton");
		$('body').removeClass('disableClickBody');
		$(".anand-loader").hide();
		return false;
	}
});
$("#zoneEdit").submit(function(){
	$("#saveZoneChanges").addClass("disabledButton");
	$('body').addClass('disableClickBody');
	$(".anand-loader").show();
	var submitOrNot = true;
	if(zoneNameValidation("Edit") == false) {
		submitOrNot = false;
	}
	if($("#zoneActiveEdit1").prop("checked") == false && $("#zoneActiveEdit2").prop("checked") == false) {
		$("#isActiveError").html("Zone state must be selected");
		submitOrNot = false;
	}
	if(submitOrNot) {
		return true;
	} else {
		$("#saveZoneChanges").removeClass("disabledButton");
		$('body').removeClass('disableClickBody');
		$(".anand-loader").hide();
		return false;
	}
});
$("#zoneName").focusout(function() {
	zoneNameValidation("");
});
$("#zoneNameEdit").focusout(function() {
	zoneNameValidation("Edit");
});
var zoneNameCheck = /^[ A-Za-z0-9-Á& ]*$/;
function zoneNameValidation(idAppend) {
	var id = "zoneName"+idAppend;
	var error = "zoneNameError"+idAppend;
	$("#"+id).removeClass("is-valid");
	$("#"+id).removeClass("is-invalid");
	$("#"+error).html("");
	var zoneName = $("#"+id).val().replace(/\s+/g, " ");
	if(zoneName != "") {
		if(zoneNameCheck.test(zoneName) == false) {
			$("#"+error).html("Zone Name cannot contain special characters.");
			$("#"+id).addClass("is-invalid");
			return false;
		} else {
			$("#"+id).addClass("is-valid");
			return true;
		}
	} else {
		$("#"+error).html("Zone Name cannot be empty.");
		$("#"+id).addClass("is-invalid");
		return false;
	}
}
$('#isActiveDiv input[type="radio"]').click(function() {
	$("#isActiveError").html("");
});
$('#isActiveEditDiv input[type="radio"]').click(function() {
	$("#isActiveEditError").html("");
});